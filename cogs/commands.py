          ...
          if "patent troll" in roles:
              ...
              if len(loser) < 1 :
                   ...
                   if "lawyer'd up" in loserroles:
                        ...
                        jsonString = json.dumps(builder, indent=4)
                        rp = requests.patch(url, headers=headers, data=jsonString)
                        mes = "After being sued by {0.author.mention}, {1} lost {2} because they were too cheap to pay their legal fees".format(ctx,person.mention,total)
                        channel = self.bot.get_channel(831400394184851456)
                        send = await channel.send(mes)
          ...
          if "little john" in roles:
               ...
               if memberCount == 0:
                    ...
                    if  "robinhood immune" in leaderRoles:
                         ...
                         mes = "Whats that {0.author.mention}? I steal from the rich and give to the needy! My name is Robinhood and i am very greedy! I took {1} from {2} to distribute amongst my Merry People.".format(ctx, total, leaderInfo.mention)
                         send = await ctx.channel.send(mes)
                         lj = discord.utils.get(ctx.guild.roles, name="Little John")
